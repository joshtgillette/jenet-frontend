  ...
  const sendMessage = async () => {
    ...
    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_BACKEND_API}/message`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
      ...
      content={
        <>
          ...
          >
            ...
            <h1 className="text-sm italic text-green-700 mt-0 ml-0.5">
              active now
            </h1>
          </div>
          <div
          ...
          >
            ...
            {sample && (
              <div className="right-10 mb-[-15px] w-full flex justify-end">
                <div className="glass max-w-[75%] mr-3 px-3.5 py-2.5 text-base break-words">
                  {sample}
                </div>
              </div>
            )}
